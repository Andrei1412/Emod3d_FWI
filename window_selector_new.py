...
class WindowSelector(object):
    ...
    def check_data_quality(self):
        ...
        len_noise = 7
        print([len(noise),len_noise, len(signal)])
...
######Modify noise to len=1
        noise_int = np.sum(noise ** 2) 
        noise_int = noise_int/len(noise)
        #noise_int = noise_int/len_noise
        noise_amp = np.abs(noise).max()
        
        signal_int = np.sum(signal ** 2) 
        signal_int = signal_int/len(signal)
        signal_amp = np.abs(signal).max()        
